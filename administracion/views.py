...
def editar(request,id):
    ...
    if request.method == 'POST':
        if request.POST['nombrenoticia'] !="" and request.POST['idequipo'] !="" and request.POST['descripcion']!="" and request.POST['fecha']!="" and request.POST['urlimagen']!="":
            competencia = Competencia.objects.get(pk=request.POST['idcompetencia'])
...
def guardar(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['nombrenoticia'] !="" and request.POST['idequipo'] !="" and request.POST['descripcion']!="" and request.POST['fecha']!="" and request.POST['urlimagen']!="":
            competencia = Competencia.objects.get(pk=request.POST['idcompetencia'])
            nuevanoticia = Noticia(nombrenoticia=request.POST['nombrenoticia'], idcompetencia=competencia,tiponoticia=competencia.nombrecompetencia,equipo=request.POST['idequipo'],subtitulonoticia="",idnoticia=obj.idnoticia+1) 
            nuevanoticia.save()
            nuevodetalle = Detallenoticia(idnoticia=nuevanoticia,descripcionnoticia=request.POST['descripcion'],fechanoticia=request.POST['fecha'],urlimagen=request.POST['urlimagen'],iddetalle=objdetalle.iddetalle+1)
            nuevodetalle.save()
            messages.success(request, "Guardado Con exito")
            return redirect('noticiasCRUD')
        else:
...
def eliminar(request):
    if request.method == 'POST': 
        Detallenoticia.objects.filter(idnoticia=request.POST['idnoticia']).delete()
